...
public class SLinkedList {
	 protected Node head;	 // head node of the list
	  ...
	  //returns boolean if tail is found, if not returns false (similar to gotoHead())
	  public boolean gotoTail(){
		  
		  if(isEmpty()){//if the list is empty nothing to return
			  return false;
		  }
		  
		  curr=tail;//if list is not empty, the current points to the tail
